...
function getExistingTerms(): Set<string> {
  ...
  const entries: Array<{ term: string, aliases?: string[] }> = [
    ...
    { term: 'Git' },
    { term: 'Docker' },
    { term: 'winget', aliases: ['Windows Package Manager'] },
...
function extractCandidates(body: string): Map<string, number> {
  ...
  const mixed = body.match(/\b[a-z]+[A-Z][a-zA-Z]+\b/g) || []
  ...
  // 数字 + 英文（如 C4D）
  const numWords = body.match(/\b[A-Z]+\d[A-Z0-9]*\b/gi) || []
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      for (const [term] of candidates) {
        if (!termArticles.has(term))
          termArticles.set(term, [])
  ...
  const candidates = [...allTermArticles.entries()]
    .filter(([term, articles]) => {
      ...
      if (existingTerms.has(term))
        return false
      // 排除常见英文单词、单字母等
      if (/^[A-Z][a-z]{0,2}$/.test(term) && !isCommonWord(term) && term.length <= 2)
        return false
