...
**cgart-analysis** ：CG 艺术实验室降维研究框架。这是我遇到陌生领域时的首选工具。它的核心价值不仅告诉你「是什么」，更追溯「怎么走到这一步」以及「未来会往哪里去」。

执行流程分三阶段：第一阶段通过子 Agent 并行分布式搜索，从纵向脉络（时间轴还原完整生命故事）、横向格局（竞争定位、差异化价值）、跨领域连接三个角度获取信息；第二阶段写出 25000-35000 字的深度分析报告；第三阶段进行交叉洞察，产出超越单一视角的创造性判断。最终报告按照专业美学标准生成，底色暖米纸，强调色油墨蓝。
...
核心价值是把信息过滤从「大海捞针」变成「智能筛选」。采集阶段支持三个通道：Telegram（每频道最多 20 条）、RSS（每源最多 10 条）、搜索（每关键词最多 5 条）。评估阶段通过 Filter Agent 和 Challenge Agent 进行语义辩论，争议无法达成共识时保留，交由读者判断。最终只把真正有价值的资讯交付给我，而不是每天面对成千上万条噪音。
