...
![](../_images/主题%20-%20No.19%20玄光周刊-1790100000000.webp)
...
**cgart-analysis** ：CG艺术实验室降维研究框架。这是我遇到陌生领域时的首选工具。它的核心价值不仅告诉你「是什么」，更追溯「怎么走到这一步」以及「未来会往哪里去」。

执行流程分三阶段：第一阶段通过子 Agent 并行分布式搜索，从纵向脉络（时间轴还原完整生命故事）、横向格局（竞争定位、差异化价值）、跨领域连接三个角度获取信息；第二阶段写出25000-35000字的深度分析报告；第三阶段进行交叉洞察，产出超越单一视角的创造性判断。最终报告按照专业美学标准生成，底色暖米纸，强调色油墨蓝。

当我需要快速了解一个陌生的领域、产品、品牌、企业或人物时，这个 Skill 能在几分钟内给我一个完整的研究报告框架，而不是碎片化的搜索结果。
...
**news-report** ：多源资讯采集。这是一套从 Telegram 频道、RSS 订阅和 Brave 搜索三个通道并行抓取信息，通过多 Agent 语义辩论评估筛选，最终输出格式统一资讯汇报的系统。

核心价值是把信息过滤从「大海捞针」变成「智能筛选」。采集阶段支持三个通道：Telegram（每频道最多20条）、RSS（每源最多10条）、搜索（每关键词最多5条）。评估阶段通过 Filter Agent 和 Challenge Agent 进行语义辩论，争议无法达成共识时保留，交由读者判断。最终只把真正有价值的资讯交付给我，而不是每天面对成千上万条噪音。
...
![图片取自原文页面](../_images/那些真正扛起工作的%20Skill%20-%20No.19%20玄光周刊-1790100000001.webp)
...
![图片取自原文页面](../_images/那些真正扛起工作的%20Skill%20-%20No.19%20玄光周刊-1790100000002.webp)
...
![图片取自原文页面](../_images/那些真正扛起工作的%20Skill%20-%20No.19%20玄光周刊-1790100000003.webp)

🔗 https://codeium.com/windsurf

Windsurf 是 Codeium 推出的 AI IDE，它的**SUPERCOMPLETION** 特性让我眼前一亮。与传统代码补全不同，SUPERCOMPLETION 可以理解完整的项目上下文，生成跨多个文件的代码片段。

对于大型项目的重构和功能开发，这个功能可以大幅减少人工编码量。不过目前还处于早期阶段，实际效果因项目而已。
