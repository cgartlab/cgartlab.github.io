...
async function _getPosts(lang?: Language) {
	...
	const filteredPosts = await getCollection(
		...
		({ data }: CollectionEntry<"posts">) => {
			...
			return (
				shouldInclude && (data.lang === currentLang || data.lang === "")
			);
		},
	);
...
async function _getRegularPosts(lang?: Language) {
	...
	return posts.filter((post) => {
		if (post.data.pin) return false;
		const tags = post.data.tags || [];
		return !tags.some(
			(tag) =>
				tag.toLowerCase() === "周刊" || tag.toLowerCase() === "weekly",
		);
	});
}
...
async function _getPinnedPosts(lang?: Language) {
	...
	return posts
		.filter((post) => {
			if (!post.data.pin || post.data.pin <= 0) return false;
			const tags = post.data.tags || [];
			return !tags.some(
				(tag) =>
					tag.toLowerCase() === "周刊" ||
