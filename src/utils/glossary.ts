...
const EN_DESCRIPTIONS: Record<string, string> = {
	...
	"nat-traversal":
		"Technology for accessing internal network hosts through NAT",
	powershell:
		"Cross-platform task automation and configuration management command-line shell",
	...
	proxmox:
		"Open-source virtualization environment supporting containers and VM management",
	...
	"service-worker":
		"Browser background script for offline caching and network interception",
};
...
export function resolveDescription(entry: TermEntry, lang: Language): string {
	...
	if (/^[\x20-\x7E]+$/.test(entry.term)) return entry.term;
	// 中文术语名（如 “代理”）留空
	return "";
}
...
export function resolveTerm(entry: TermEntry, lang: Language): string {
	const override = entry.langs?.[lang];
	if (override?.term) return override.term;
	if (lang === "en") {
