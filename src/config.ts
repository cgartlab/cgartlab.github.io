...
export const themeConfig: ThemeConfig = {
	...
	site: {
		...
		// site description
		description:
			"CG艺术实验室（cgartlab.com）是一个专注于动态视觉设计、数字艺术、技术分享和知识管理的工作室。致力于为数字艺术爱好者、专业设计师和初学者以及商业客户，分享创作经验和提供服务。",
	...
	color: {
		...
		light: {
			...
			// used for navbar underline, selected text bg, etc
			// 3.10:1 contrast against light background
			highlight: "oklch(41% 0.12 138)",
		},
		dark: {
			// primary color
			...
			// highlight color - olive green
			// used for navbar underline, selected text bg, etc
			// 4.52:1 contrast against dark background
			highlight: "oklch(72% 0.14 138)",
		},
	},
	// COLOR SETTINGS >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> END
...
export const base =
	themeConfig.site.base === "/"
		? ""
		: themeConfig.site.base.replace(/\/$/, "");
